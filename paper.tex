...
Operating systems and other computational factors are known to have an
effect on the results produced by neuroimaging pipelines, presumably due to
the creation, propagation and amplification of numerical errors along data
analysis pipelines. However, the precise causes of such instabilities and
the path along which they propagate in the pipelines are unclear, while
this process could play a major role in the current reproducibility crisis.
We present a technique to identify the computational processes in a
pipeline that create numerical differences when executed in different
computational conditions. Our technique leverages system-call interception to reconstruct
processing and file graphs without pipeline instrumentation. By applying our tool to
the HCP pre-processing pipelines, 
we find that linear and non-linear registration are the cause of
...
from FSL, AFNI, Freesurfer and ANTs to provide a state-of-the art
...
Spot identifies processes that create differences from a process graph of the pipeline execution.
This tool compares file dependencies through all the processes based on the result files in two conditions.
In this paper, different conditions refer to various operating systems.
This tool is applicable on the complex pipelines in which tracing a large number of processes is difficult. 

Figure~\ref{fig:overview-tool} shows an overview of our method. For each tested
condition, Condition 1 and Condition 2, the pipeline is containerized with
Docker, and its interface is described with
Boutiques~\cite{glatard2017boutiques}. 
\reprozip is added to Condition 1 for provenance tracking. 
This captures process dependencies through system call interceptions, 
then Spot tool creates related process trees (Figure~\ref{fig:overview-tool} - (1)).
Different subjects produce different process trees. 
Hierachical clustering used to group subjects into different types (Figure~\ref{fig:overview-tool} - (2)).
The rest of the procedure is done separately on each subject type.
Our tool continues by
restoring temporary files and ``multi-write" files, i.e., files written
by more than one process (Figure~\ref{fig:overview-tool} - (3)). 
To do so, the tool creates a list of processes 
that introduce intermediary files and replaces their corresponding executables with a wrapper script. 
The tool re-executes the pipeline in a modified Docker image in Condition 1.
This copies files in the list to a read-only directory.
In the next step, the
processes are labeled from the differences found in their outputs in the
two tested conditions (Figure~\ref{fig:overview-tool} - (4)). 
The tool builds the Docker image for Condition 2 by modifying all
the processes in the pipeline. Then, the modified Docker
image is executed in Condition 2 and each process is labeled after
running by comparing the results obtained in Condition 1
and output files of the process in Condition 2. We label processes in 2
...
if the process creates differences.
Process labelling
is done through hooks which are defined during the Docker image modifying
procedure in Condition 2.
Spot tool is available at https://github.com/big-data-lab-team/repro-tools.
...
Different subjects may lead to different process trees, due to
...
Accordingly, we cluster subjects into different types. 
...
process trees are generated. 
Nodes in the process trees are labeled by the process name.
The tree edit distance~\cite{zhang1989simple} is used as a similarity measure for 
representing the distance between labeled trees. 
Tree edit distance  is defined  
...
We will show that different types of subjects certainly produce different results, 
while subjects in the same type can be either reproducible or not.
