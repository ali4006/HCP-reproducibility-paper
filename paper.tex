...
The similar findings in the other studies show that the subject 
is an important factor in reproducibility~\cite{bowring2019exploring}.

% make figure in two columns use {figure*}
\begin{figure}
\centering
  \includegraphics[width=\columnwidth,height=4cm]{images/pfs_heatmap.png}
...
\begin{figure}
  \centering
    ...
    CentOS7). Sagittal (top) and axial (bottom) views of the brain in dimension of X=130, Y=155, Z=130.
    Right images show the zoomed version part of the images marked by the red rectangle.
    Here is 
...
Furthermore, Figure~\ref{fig:scatter_plot} plots the mean of Dice coefficients 
for the 45 segmented regions obtained on two operating systems.
...
\centering
  ...
  \caption{Heatmap binarized differences between brain segmentation results (file \emph{aseg.hires.mgz}) from 
          ...
          {the link} to the gif animation from a single subject.} 
...
\begin{figure}
  \centering
    ...
    \caption{The scatter plot of the mean of Dice coefficients between segmented regions 
            from 20 subjetcs in FreeSurfer (CentOS6 vs. CentOS7). 
            The complete Dice coefficient values are 
...
\note{FREESURFER RESULTS: 
These are consistant results to the paper~\cite{Glatard2015}, as it is shown the propaggation of differences in cortical thickness extraction analysis.
The magnitude of differences from the operating system changes is out of our expectaion}
