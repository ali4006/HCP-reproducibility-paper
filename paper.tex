...
In our experiments, we identified two processes that create differences due to 
within-OS differences.  
Registration process with \emph{mri\_segreg} and process \emph{mris\_curvature} that 
computes mean and Gaussian curvatures of a cortical surface using 
the random variable in FreeSurfer.
To avoid such pseudo-randomness, we 
initialized random number generators using a seed state in 
the command line in which pipeline script calls the process.
In this way, NURM-tool identifies the processes in the pipelines 
that create between-OS differences. 
...
Among our dataset, we identified four types of subjects with 
the different topology of \emph{process trees}. 
Differences correspond to the data processing that leads to producing a disparate output file(s).
...
The frequency of the processes that create differences in 
the pipeline among 20 subjects is determined in Figure~\ref{fig:pfs_freq}. 
\emph{FLIRT} is the most frequent process that produces differences.
...
Results show that \emph{fslmaths} and \emph{FLIRT} 
...
We created checkerboard images from the image results of one subject in two OSes.
...
Each rectangle includes the pixels with different intensity values between images.
...
This process generates surface files for cortical and white matter surfaces (see~\cite{} for more details).
...
In the second test, the results of PreFreeSurfer feed to the FreeSurfer 
...
Therefore, the magnitude of differences is in the range of 1-20 as the total number of subjects.
...
For this purpose, we can evaluate the final result after perturbing the identified processes in the pipeline iteratively.
