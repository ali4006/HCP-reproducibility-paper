...
We reconstruct
the tree of processes starting from the first process created by the pipeline and retrieving its child processes with dependency files in
the next levels from database. \todo{Start here.} The process tree of two subjects has
tested in the same pipeline conditions which are similar, so we
collect such data only for one subject (The term condition refers to
the variation of operating systems on which the data processing takes
place). Furthermore, the process tree is acyclic “with the respect of
this assumption that there is no race conditions between processes to
