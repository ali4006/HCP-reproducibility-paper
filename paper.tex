...
by more than one process (Fig.~\ref{fig:overview-capturing}) \tristan{check if concurrent writes are discussed}. To do that,
the tool first executes the pipeline in Condition 1, to obtain a process
graph (Fig.~\ref{fig:overview-capturing} - (P)) and a list of result files.
The list of processes that create intermediary or multi-write
files is then generated. Using this list, the tool modifies Docker image in Condition 1 and
exectutes the pipeline again through it, which turns to produce results
including temporary and overwritten files
(Fig.~\ref{fig:overview-capturing} - (C1)). In the second step, the
...
processes that recognised from the process graph. Then, the modified docker
...
(Fig.~\ref{fig:overview-labelling} - (D)). We then label processes in 2
categories (Fig.~\ref{fig:overview-labelling} - (L)) depending whether they
create differences (represented in red) or not (green). Process labelling
...
\tristan{Fig 1-a isn't in the git repo while I had asked it before and it is 
written in the getting started guide anyway. It is annoying to have to ask for these things
multiple times while I would like to focus on the content. Also, ppt is among the worst formats
to put in a git repo as it doesn't play well with diffs.}

\tristan{In Fig 1-a, change ``overwritten'' to ``multi-write''.}

\tristan{We should also discuss the cardinality of ``input subject''. Are we talking
about 1 subject or all subjects?}
...
We use \reprozip (Fig.~\ref{fig:overview-capturing} - (P))
...
and \texttt{fork()} dependencies. Our tool also creates a process \emph{graph} from this tree
...
\tristan{You should use Fig. or Figure consistently}
...
\tristan{Add more comments, in particular to explain what the variables represent.}
This 
...
\begin{listing}
  ...
  p_exec_cond2 = p_exec_cond2 + " ".join(sys.argv[1:])
  ...
  if p_exec_cond1 in process_list.keys():
      # Make backup copy
      capture_file(p_files, from_cond1, to_backup)
  
  \end{minted}
    \caption{Modified version of the script in Listing~\ref{listing:sample-script} to save temporary files.}
