...
different conditions are outlined in Figure~\ref{fig:overview-tool}. First,
...
edges represent read or write file accesses (Step 1 in Figure
Figure~\ref{fig:overview-tool}). \emph{Transient} files are then identified as
...
Condition 2, in a step-by-step process that prevents the propagation of differences.
...
\subsection{Recording of provenance graphs}
...
Provenance graphs may be subject-dependent, which justifies iterating
\toolname on multiple subjects. For instance, data acquisitions may be
...
some subjects only, but other differences cannot be ignored, for instance
...
\subsection{Capture of transient files}
...
without pipeline instrumentation, by pre-pending to the PATH environment
variable a directory containing a wrapper script named as the executable
called by the process (see Listing~\ref{listing:modify-script}).

% The temporary files that are removed by P 
% itself won't be saved, but this is not a problem since these files do not play any role in 
...
\begin{listing}
  ...
  \end{minted}
    \caption{Wrapper script used to save temporary and modified files.}
...
Files written by multiple processes are disambiguated as follows. For a
 ...
 calls $P_{i}$ and then saves F to a read-only directory. In this way, the
 multiple versions of F are preserved for comparison. \toolname finally
 ...
 acyclic, since we assumed that a process cannot modify a file once it was
 written \todo{check/prove that}.
...
\subsection{Process labelling} 

Once all the files have been captured in Condition 1, including transient
files, \toolname labels pipeline processes through a step-by-step execution
...
irrreproducible and the output files produced in Condition 1 are copied to
...
of the \texttt{PATH} variable similar to the one described previously. 

By default, differences in output files are identified by comparing file
...
Finally, \toolname creates a labeled provenance graph, 
