...
\revised{
The labeled graph can be different depending on the order of executions in which condition 
we execute pipeline to pinpoint the propagation of differences. 
Therefore, we label processes in two order of executions, and then aggregate findings so that label processes 
as non-reproducible (red) if it creates different results in both order of executions.
}
...
\section{Results}
\revised{
All experiments were run on a machine with a 3.4GHz Intel Core i7 processor and 32GB memory 
under the CentOS7 operating system. 
The details of the computation of the PreFreeSurfer and FreeSurfer pipelines are illustrated 
in~Table~\ref{table:pipeline-stats}.
...
\begin{table*}[ht]
  ...
  \caption{The computation details of the pipelines per subject.}
  ...
  Processing time (mins)      &     106.67     &     2.68     &     nd     &     nd     \\
  Output file size (GB)       &     2.8        &     0.10     &     nd     &     nd     \\
  Number of file accesses     &     94089      &     2644.99  &     nd     &     nd     \\
  Number of processes         &     8731       &     198.18   &     nd     &     nd     \\
  \bottomrule
  \end{tabular}
  \end{threeparttable}
  \end{table*}
...
\centering
  ...
  17 subjects. 
  \revised{Grey cells show that the process did not occur in the pipeline step.}}
...
\begin{figure}
  \centering
    ...
    % Result images obtained from CentOS6 and CentOS7 are colored in purple and green squares.
    % The red boxes highlight regions with significant differences.}
...
\revised{However, there is no significant correlation between Dice value and region size.}
...
\centering
  ...
  template. 
  \revised{
  The binary images are created based on the intensity differences between result images in CentOS6 and CentOS7. 
  The voxel is 1 if the intensity value of the corresponding voxel in the images is different, otherwise it is 0.
  Each voxel shows the number of subjects that created different results. 
...
\revision{To evaluate the reproducibility of a pipeline, \toolname needs to execute
