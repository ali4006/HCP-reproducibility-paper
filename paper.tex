...
\tristan{Ali: please review these assumptions and check if any other one is required}
...
to file reads and file writes (Figure~\ref{fig:simple_script}-left). \tristan{the new example is 
confusing: it has too many processes and its purpose is confusing. This should be simplified. 
See suggestion in example.sh: it has only a few processes, and it has a temporary and a multiple-write file. 
If you agree that this example is enough, could you update the figure?}
...
\centering
  ...
  system calls) \tristan{Does spot make any use of the process tree at any stage? If not, I think we may want to remove these edges.}. Dashed edges represent file dependencies.
...
\tristan{Ali, are these numbers per subject or for 20 subjects?}
...
compares results of ACPC-Alignment in PreFreeSurfer. \tristan{Ali, there is no
FNIRT process in ACPC-Alignment, please check.}
...
dynamically linked?} This process generates the cortical and white matter
