...
\note{Ali}{It only shows processes that belong to the pipeline (e.g. /usr/local/src/freesurfer/) 
by making a query on the traced DB file. 
We exclude processes like [cp, awk, sed, tee, date\dots]. \tristan{Several points here: (1) freesurfer is obviously not used in this example, so what filtering was actually done? (2) where is this implemented in the code? (3) 
how do you know that you're not breaking the graph by removing these processes?}}
\note{Ali}{In fact, I excluded processes in /usr/bin/ like [cp, sed, awk, tee, ...] in \emph{get\_processes\_list} function in \emph{auto\_spot.py} script. These processes will not affect the outputs as the results are shown. 
Spot will stop the procedure of labeling if any excluded processes create differences as it cannot labels included processed that read and write files with differences. It didn't happen in my experiments.
}
...
\begin{figure*}
  ...
  \end{subfigure}
    ...
    plain edges. Every node in the graph is labeled using (1) a process id
...
\begin{minipage}{\linewidth}
  \centering
    ...
    \caption{A complete provenance graph from the PreFreesurfer pipeline. \tristan{Ali, the process tree needs to be in dashed and the file
    dependencies in plain.} \note{Ali}{This figure is consistent with Fig.1. plain edges show process tree and dashed edges are read/write accesses. I just changed the caption in Fig.1.}}
