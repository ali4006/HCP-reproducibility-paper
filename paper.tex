...
\emph{labeled process tree} are generated. This implies the effect of different 
subjects on pipeline results. First, we cluster subjects based on their process 
trees that are labeled by the process name. Then, we cluster subjects in each 
category after re-labeling the processes that create differences. In this order, 
the tree edit distance~\cite{zhang1989simple} is used as similarity measure for 
...
number of occurrences are depicted in Figures~\ref{fig:pfs_table} and 
~\ref{fig:pfs_chart} including linear registration with “\emph{FLIRT}” (in 
