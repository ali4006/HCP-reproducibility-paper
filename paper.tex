...
\toprule
Type   & Number of Subjects  & Available Modalities                                         \\ \midrule
type1  & 9                   & [``T1w\_MPR1", ``T1w\_MPR2",                     \\
&  &                            ``T2w\_SPC1", ``T2w\_SPC2"]     \\
type2  & 8                   & [``T1w\_MPR1", ``T2w\_SPC1"]                                 \\
type3  & 1                   & [``T1w\_MPR1", ``T2w\_SPC1",   \\
&  &                            ``T2w\_SPC2"]                   \\
type4  & 2                   & [``T1w\_MPR1", ``T1w\_MPR2",   \\
&  &                            ``T2w\_SPC1"]                   \\ \bottomrule
\end{tabular}
\begin{tablenotes}
     \small
...
\centering
  ...
  \caption{Heatmap frequency of processes along with the sub-pipelines that introduce 
           differences among 20 subjects. The range between red and green colors shows the most and 
           least frequency of processes, respectively. 
           Each cell describes the frequency of the specific process in the sub-pipelines.        
           Each cell may split into multiple sub-cells when the specific process 
           called by different options (distinct process).
           Annotations represent `N [M] x Y" in which N is the percentage of the distinct process 
           that creates differences out of all M times that is occurred in that sub-pipeline. 
           Also, times Y shows the number of the distinct process with the same distribution.
           For example, the cell of process FLIRT in sub-pipeline AAve shows 10 distinct processes  
...
\begin{figure}
  \centering
    ...
    Here is 
    \href{https://raw.githubusercontent.com/ali4006/HCP-reproducibility-paper/master/images/brain_classification.gif}
    {the link} to the gif animation of the same images.
} 
...
\centering
  \includegraphics[width=\columnwidth]{images/brain_classification_mni.png} 
  \caption{Heatmap binarized differences between brain segmentation results (file \emph{aseg.hires.mgz}) from 
          ...
          We calculated some statistics of the differences based on the dice value of the segmented regions.
          Here is 
          \href{https://encsconcordiaca-my.sharepoint.com/:x:/g/personal/m_alari_encs_concordia_ca/EfNB-QMcivhIv6j0r9weffwBCHujf8RbaKjiYalIFa8cbQ?e=hyvQSU}
