...
except those that their binaries are placed in directory \texttt{/usr/bin/} or \texttt{/bin/}.
...
\begin{figure*}
  ...
  \end{subfigure}
    ...
    dashed edges. \note{Ali}{Every node in the graph is labeled using the name of the executable run by the
    process or the file name. All the processes are the calls to FSL bet,
    stats and maths made in Listing~\ref{listing:sample-script}. Process \texttt{bet2}
    is forked by process \texttt{bet}: it was captured by \reprozip while it did not
...
\begin{minipage}{\linewidth}
  \centering
    \includegraphics[width=.6\linewidth]{figures/pfs-labeled.pdf} 
    \caption{A complete provenance graph from the PreFreesurfer pipeline.
   \note{Ali}{ We excluded six more processes in the pipeline for better visualization, including:
